...
class SingleArm(Manipulator):
    ...
    def __init__(
        ...
        self.ee_reaction = np.zeros(6)
        self.ee_action = np.zeros(6)
    ...
    def control(self, action, policy_step=False):
        ...
        gripper_action = None
        scaling_up = 1.0  # scaling up from the output of tanh (around -0.5 to 0.5)

        if not self.isTele:
            action = scaling_up * action
        ...
        if policy_step:
            ...
            self.ee_action = arm_action

        # Now run the controller for a step
        self.torques, self.ee_reaction = self.controller.run_controller()
    ...
    def setup_observables(self):
        ...
        # WAVE control features
        @sensor(modality=modality)
        def eef_reaction_v(obs_cache):
            return self.ee_reaction[:3]

        @sensor(modality=modality)
        def eef_reaction_w(obs_cache):
            return self.ee_reaction[3:]
        ...
        def eef_force(obs_cache):
            if self.force_count < 10:
                ...
                """
                self.ee_force_bias = (self.ee_force_bias*self.force_count) + self.ee_force
                self.force_count = self.force_count + 1
                self.ee_force_bias = self.ee_force_bias/self.force_count
                """
                return np.zeros(3)
            else:
                return self.ee_force - self.ee_force_bias

        @sensor(modality=modality)
        def eef_torque(obs_cache):
            if self.torque_count < 10:
                self.ee_torque_bias = self.ee_torque
                self.torque_count = self.torque_count + 1
                """
                self.ee_torque_bias = (self.ee_torque_bias*self.torque_count) + self.ee_torque
                self.torque_count = self.torque_count + 1
                self.ee_torque_bias = self.ee_torque_bias/self.torque_count
                """
                return np.zeros(3)
            else:
                return self.ee_torque - self.ee_torque_bias

        @sensor(modality=modality)
        def eef_action_v(obs_cache):
            return self.ee_action[:3]

        @sensor(modality=modality)
        def eef_action_w(obs_cache):
            return self.ee_action[3:]
        ...
        sensors = [eef_reaction_v, eef_reaction_w, eef_force, eef_torque, eef_action_v, eef_action_w, eef_pos, eef_quat, eef_vel_lin, eef_vel_ang]
        names = [f"{pf}eef_reaction_v", f"{pf}eef_reaction_w", f"{pf}eef_force", f"{pf}eef_torque", f"{pf}eef_action_v", f"{pf}eef_action_w", f"{pf}eef_pos", f"{pf}eef_quat", f"{pf}eef_vel_lin", f"{pf}eef_vel_ang"]
    ...
    def ee_force(self):
        ...
        """
        Returns:
            np.array: force applied at the force sensor at the robot arm's eef, EXPRESSED IN THE WORLD FRAME
        """
        #return self.get_sensor_measurement(self.gripper.important_sensors["force_ee"])
        #return self.sim.data.get_body_xmat(self.robot_model.eef_name).dot(self.get_sensor_measurement(self.gripper.important_sensors["force_ee"]))

        temp_force = self.get_sensor_measurement(self.gripper.important_sensors["force_ee"])
        temp_force[2] = -temp_force[2]
        return self.sim.data.get_body_xmat(self.robot_model.eef_name).dot(temp_force)

    @property
    def ee_force_bias_subtracted(self):
        if self.force_count < 10:
            return np.zeros(3)
        else:
            return self.ee_force - self.ee_force_bias
    ...
    def ee_torque(self):
        ...
        """
        Returns torque applied at the torque sensor at the robot arm's eef, EXPRESSED IN THE WORLD FRAME
        """
        #return self.get_sensor_measurement(self.gripper.important_sensors["torque_ee"])
        #return self.sim.data.get_body_xmat(self.robot_model.eef_name).dot(self.get_sensor_measurement(self.gripper.important_sensors["torque_ee"]))

        temp_torque = self.get_sensor_measurement(self.gripper.important_sensors["torque_ee"])
        temp_torque[2] = -temp_torque[2]
        return self.sim.data.get_body_xmat(self.robot_model.eef_name).dot(temp_torque)
