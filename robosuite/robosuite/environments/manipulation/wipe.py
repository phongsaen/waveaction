...
DEFAULT_WIPE_CONFIG = {
    ...
    "table_friction_std": 0.3,  # Standard deviation to sample different friction parameters for the table each episode
...
class Wipe(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self.print_results:
            string_to_print = (
                ...
                "wiped markers: {ws:>3} contact: {ct:>1} pressing force: {pf:8.4f} collisions: {sc:>3} f-excess: {fe:>3}".format(
                    pid=id(multiprocessing.current_process()),
                    ts=self.timestep,
                    rw=reward,
                    ws=len(self.wiped_markers),
                    ct=self._has_gripper_contact,
                    #pf=self.robots[0].ee_force[2],
                    pf=self.robots[0].ee_force_bias_subtracted[2],
                    sc=self.collisions,
                    fe=self.f_excess,
                )
            )
    ...
    def _post_action(self, action):
        ...
        # Update force bias
        # not necessary, since we now call eef_force_bias_subtracted() of which the force bias has been subtracted
        """
    ...
    def _has_gripper_contact(self):
        ...
        return np.linalg.norm(self.robots[0].ee_force_bias_subtracted) > self.contact_threshold
