...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def step(self, action):
        ...
        for i in range(int(self.control_timestep / self.model_timestep)):
            self.sim.forward()
            v_s = self._pre_action(action, policy_step)
            self.sim.step()
        ...
        observations = self.viewer._get_observations() if self.viewer_get_obs else self._get_observations(force_update=True)

        return observations, reward, done, info, v_s
