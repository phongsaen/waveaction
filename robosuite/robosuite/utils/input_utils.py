...
def input2action(device, robot, active_arm="right", env_configuration=None):
    ...
    elif controller.name == "WAVE" or controller.name == "WAVE_INT" or controller.name == "WAVE_OSC":
        # Scale rotation for teleoperation (tuned for WAVE) -- gains tuned for each device
        drotation = drotation * 1.5 if isinstance(device, Keyboard) else drotation * 0.1
        dpos = dpos * 75 if isinstance(device, Keyboard) else dpos * 0.1

    else:
        # No other controllers currently supported
        print("Error: Unsupported controller specified -- Robot must have either an IK or OSC-based or WAVE controller!")
