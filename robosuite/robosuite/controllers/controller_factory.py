...
from .wave_int import WaveInterpolationController
from .wave_osc import WaveOSCController
...
def controller_factory(name, params):
    ...
    if name == "WAVE":
        ...
        if interpolator is not None:
            ...
            ori_interpolator.set_states(ori="euler")
        params["control_ori"] = True
        return WaveTeleoperationController(interpolator_pos=interpolator, interpolator_ori=ori_interpolator, **params)

    if name == "WAVE_INT":
        if interpolator is not None:
            interpolator.set_states(dim=6)  # dim of wave command = 6
        params["control_ori"] = False
        return WaveInterpolationController(interpolator=interpolator, **params)

    if name == "WAVE_OSC":
        if interpolator is not None:
            interpolator.set_states(dim=6)  # dim of wave command = 6
        params["control_ori"] = False
        return WaveOSCController(interpolator=interpolator, **params)
