...
class WaveOSCController(Controller):
    """
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        # wave impedance
        self.b_v = 75.0  #200.0  #75.0  #200.0  #300.0
        self.b_w = 75.0  #200.0  #75.0  #200.0  #150.0
        # input wave
        self.u_s = np.zeros(6)
        # output wave
        self.v_s = np.zeros(6)
    ...
    def run_controller(self):
        ...
        # 6. Compute desired force (F_sd) and torque (tau_s) based on errors
        position_error = self.pos_sd - self.pos_s
        vel_pos_error = -self.vel_s[0:3]
        ...
        ori_error = orientation_error(T.quat2mat(self.ori_sd), self.ee_ori_mat)
        vel_ori_error = -self.vel_s[3:6]
        ...
        self.tau_s = np.dot(self.J_full.T, decoupled_wrench) + self.torque_compensation
        ...
        # recompute decoupled_wrench to have no local damping
        desired_force = desired_force - np.multiply(vel_pos_error, self.kd[0:3])
        desired_torque = desired_torque - np.multiply(vel_ori_error, self.kd[3:6])
        ...
        # compute F_sd
        self.F_sd[0:3] = decoupled_wrench[0:3] + 0.3*self.b_v*self.vel_sd[0:3]
        self.F_sd[3:6] = decoupled_wrench[3:6] + 0.3*self.b_w*self.vel_sd[3:6]
