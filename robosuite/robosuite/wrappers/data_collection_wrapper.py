...
class DataCollectionWrapper(Wrapper):
    def __init__(self, env, directory, collect_freq=1, flush_freq=100):
        ...
        self.v_m = []  # v_m observation needed to determine action
    ...
    def step(self, action, v_m_prev):
        ...
        """
        observations, reward, done, returned_info, v_s = super().step(action)
        self.t += 1
        ...
        if self.t % self.collect_freq == 0:
            ...
            info = {}
            info["action"] = np.array(action)
            self.action_infos.append(info)

            self.v_m.append(np.array(v_m_prev))
        ...
        return observations, reward, done, returned_info, v_s
