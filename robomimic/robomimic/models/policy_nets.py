...
class TransformerActorNetwork(MIMO_Transformer):
    ...
    def forward(self, obs_dict, actions=None, goal_dict=None):
        ...
        """
        mod = list(obs_dict.keys())[0]
        if len(obs_dict[mod].shape) == 2 or len(obs_dict[mod].shape) == 4:  # for single low_dim or rgb data
            # single obs --> expand to have time dimension
            obs_dict = TensorUtils.unsqueeze_expand_at(obs_dict, size=1, dim=1)
        if self._is_goal_conditioned:
            assert goal_dict is not None
            # repeat the goal observation in time to match dimension with obs_dict
            mod = list(obs_dict.keys())[0]
        ...
        # apply tanh squashing to ensure actions are in [-1, 1]
        #outputs["action"] = self.output_scaling * torch.tanh(outputs["action"])
        #outputs["action"][:, :, 6] = outputs["action"][:, :, 6] / self.output_scaling
        #outputs["action"][:, :, 6] = torch.tanh(outputs["action"][:, :, 6])
