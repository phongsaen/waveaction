...
def run_rollout(
    ...
    try:
        for step_i in range(horizon):
            ...
            # play action
            ob_dict, r, done, _, _ = env.step(ac)
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, obs_normalization_stats=None):
    ...
    for k in LogUtils.custom_tqdm(range(num_steps)):
        ...
        # update the scheduler after finishing each batch
        """
        if not validate:
            model.during_epoch(epoch, k, num_steps)
        """
