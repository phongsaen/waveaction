...
def extract_trajectory(
    ...
    traj = dict(
        ...
        rewards=[], 
        dones=[], 
        actions=np.array(actions),  # normalized action for the label of the network model
    ...
    for t in range(1, traj_len + 1):
        ...
        """
        if t < traj_len:
            state_playback = env.get_state()["states"].flatten()
            if not np.all(np.equal(states[t], state_playback)):
                err = np.linalg.norm(states[t] - state_playback)
                print(f"[Warning] playback diverged by {err:.2f} at step {t-1}")
...
def dataset_states_to_obs(args):
    ...
    for ind in tqdm(range(len(demos))):
        ...
        actions = f["data/{}/actions".format(ep)][()]  # raw action

        scaling_down = 1.0  # scaling down from the raw action to around [-0.5, 0.5]
        actions = scaling_down * actions  # normalized action for only the first six components
